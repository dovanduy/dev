<?php
// php blogger.php
// php /home/vuong761/public_html/shell/vuongquocbalo/blogger.php
include ('base.php');
...
if (file_exists('google_login.serialize')) {  
    ...
    if (empty($AppUI->google_access_token) || strtotime($AppUI->access_token_expires_at) < time()) {
        batch_info('Token is invalid');        
    } else {
        $siteUrl = 'http://vuongquocbalo.com';
        $track = 'utm_source=blogger&utm_medium=social&utm_campaign=product';    
        $limit = 5;   
        $blogs = app_bloggers();
        $result = array();
        foreach ($blogs as $blogId => $info) {      
            ...
            foreach ($products as $row) {             
                ...
                $product['url'] = $siteUrl . '/' . name_2_url($product['name']) . '?' . $track;
