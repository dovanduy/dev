...
initJsModalDialog = function () {
    if ($('.show-model').length > 0) {  
        $('.show-model').click(function(){
            ...
            var modelTitle = $(btn).data('modal_title');
            if ($(modelid).length > 0) {  
				if (modelTitle) {
					$(modelid + ' .modal-title span').html(modelTitle);
				}
                $(modelid).modal('show');     
...
initJs = function () {
	...
	$('#searchArea #showSearchFormBtn').click(function(){
		$('#searchForm #q').focus();
    });	
	...
	if ($('#searchForm404 #keyword').length > 0) {
		 $('#searchForm404').submit(function(){
			var q = $('#searchForm404 #keyword').val();
			if (q) {
				location.href = '/search/'+q;
			}
			return false;			
    ...
    */
    ...
    if ($('.price').length > 0 && $('.price').is( "input" )) {  
...
showLoading = function(e) {
	...
	img.src = 'http://vuongquocbalo.com/web/images/loading.gif';	
	div.id = 'divLoading';
