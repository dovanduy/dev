...
class Module {
    ...
    public function onBootstrap(MvcEvent $e) {
        ...
        $eventManager->attach(
                [MvcEvent::EVENT_RENDER_ERROR, MvcEvent::EVENT_RENDER], new RenderEventListener()
        );

        $eventManager->getSharedManager()->attach(
                'Zend\Mvc\Application', 'finish', array($this, 'compressHtml'), 1002
        );
    }
    ...
    public function onDispatch($e) {
        ...
        if (empty($AppUI) && 1==0) {
            ...
            if (!empty($remember['email'])) {
                if (!empty($remember['password'])) {
                    $sm->get('auth')->authenticate($remember['email'], $remember['password'], 'web');
                    $AppUI = $sm->get('auth')->getIdentity();
                } elseif (!empty($remember['facebook_id'])) {
                    return $e->getTarget()->redirect()->toRoute('web/fblogin');
                } elseif (!empty($remember['google_id'])) {
                    return $e->getTarget()->redirect()->toRoute('web/glogin');
                }
            }
        }
        ...
        $e->getTarget()->layout()->setVariable('keyword', $e->getRouteMatch()->getParam('q', ''));
    }
            ...
            'factories' => array(
                ...
                'requestHelper' => function($sm) {
                    ...
                    /*
                      ...
                      $helper->setRequest($app->getRequest());
                      $helper->setMvcEvent($app->getMvcEvent());
                      return $helper; */
                },
