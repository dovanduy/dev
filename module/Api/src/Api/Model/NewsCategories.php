...
class NewsCategories extends AbstractModel {
    ...
    protected static $tableName = 'news_categories';
    ...
    public function getList($param)
    {
        ...
        $select = $sql->select()
            ...
            ->join(
                ...
                static::$tableName . '.category_id = news_category_locales.category_id',
                array('name', 'short')
            )
            ->join(
                'news_category_images', 
                static::$tableName . '.image_id = news_category_images.image_id',
                array('url_image'),
                \Zend\Db\Sql\Select::JOIN_LEFT    
            )
            ->where("news_category_locales.locale = ". self::quote(\Application\Module::getConfig('general.default_locale')));  
        ...
        if (!empty($param['name'])) {
            $select->where(new Expression("news_category_locales.name LIKE '%{$param['name']}%'"));
        }
        ...
        if (!empty($param['sort'])) {
            ...
            if (count($match) == 3) {
                switch ($match[1]) {
                    case 'name':
                        $select->order("news_category_locales.{$match[1]} " . $match[2]);
                        break;
                    case 'sort':
    ...
    {
        $_id = mongo_id();  // news_categories._id
        if ($_FILES) {
        ...
        if ($id = self::insert($values)) {
            ...
            self::$tableName = 'news_category_locales';
    ...
    public function addUpdateLocale($param)
    {
        ...
        static::$tableName = 'news_category_locales';
        $values = array();
        if (isset($param['name'])) {
